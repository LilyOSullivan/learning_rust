...
pub fn wsp_<P: Parser>(p: P) -> impl Parser<Out = P::Out> {
    middle(WSL.istar(), p, WSL.istar())
}

parser! {   (UniEscape->char)
    ("\\u",HexDigit.exact(4).brk()).try_map(|(_,v)| {
        let n:u32 = u32::from_str_radix(&v,16).map_err(|_| Expected::Str("4 Hexadecimal digits"))?;
...
parser! { (JsonValue->Value)
    wsp_( or!(
        ...
        "{".ig_then(sep_until_ig(wsp_(MapItem),",","}")).map(|a| {
...
parser! { (MapItem->(String,Value))
    (JsonString,wsp_(":"),JsonValue).map(|(a,_,b)|(a,b))
}
